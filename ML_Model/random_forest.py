...
def random_forest(fraud_file_path):
    ...
    fraud_model = RandomForestRegressor(random_state=1)
    ...
    predictions_df.to_csv("Results/random_forest_results.csv", index=False)
    perf_df.to_csv("Results/random_forest_performance.csv", index=False)
    print("Voici les prédictions :")
    print(predictions_df)
    print("\nLes prédictions ont été sauvegardées dans le dossier Results et le fichier random_forest_results.csv")
    print("Les performances du modèle ont été sauvegardées dans le dossier Results et le fichier random_forest_performance.csv")