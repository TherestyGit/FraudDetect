...
from xgboost import XGBRegressor # erreur à ne pas prendre en compte -> Erreur locale d'importation mais ça fonctionne
from sklearn.preprocessing import LabelEncoder
from sklearn.metrics import mean_squared_error, r2_score

def xgboost(fraud_file_path):
    ...
    fraud_model = XGBRegressor(random_state=1)
    ...
    predictions_df.to_csv("Results/xgboost_results.csv", index=False)
    perf_df.to_csv("Results/xgboost_performance.csv", index=False)
    print("Voici les prédictions :")
    print(predictions_df)
    print("\nLes prédictions ont été sauvegardées dans le dossier Results et le fichier xgboost_results.csv")
    print("Les performances du modèle ont été sauvegardées dans le dossier Results et le fichier xgboost_performance.csv")